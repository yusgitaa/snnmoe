...
class SpikeDrivenTransformer(nn.Module):
    ...
    def forward_features(self, x, orig_images=None, hook=None, epoch=None, batch_idx=None, img_indices=None):
        block = getattr(self, f"block")
        patch_embed = getattr(self, f"patch_embed")

        x, _, hook = patch_embed(x, orig_images=orig_images, hook=hook)
        for blk in block:
            x, _, hook = blk(
                x, 
    ...
    def forward(self, x, orig_images=None, hook=None, epoch=None, batch_idx=None, img_indices=None):
        ...
        if not self.use_moe_sps and not self.use_moe_mlp:
            ...
            x, hook = self.forward_features(x, orig_images=orig_images, hook=hook, epoch=epoch, batch_idx=batch_idx, img_indices=img_indices)
            x = self.head_lif(x)
            if hook is not None:
            ...
            if SpikeDrivenTransformer._print_count == 0:
                ...
                print("2. Orig images shape:", orig_images.shape if orig_images is not None else "None")
                ...
                if len(x.shape) == 4:  # [B, C, H, W]
                    ...
                    print("3. After time dimension:", x.shape)
            ...
            x, hook = self.forward_features(x, orig_images=orig_images, hook=hook, epoch=epoch, batch_idx=batch_idx, img_indices=img_indices)
            if SpikeDrivenTransformer._print_count == 0:
                print("4. After forward_features:", x.shape)
            
            x = self.head_lif(x)
            if SpikeDrivenTransformer._print_count == 0:
                print("5. After head_lif:", x.shape)
            ...
            x = self.head(x)
            if SpikeDrivenTransformer._print_count == 0:
                print("6. After head:", x.shape)
            
            if not self.TET:
                x = x.mean(1)  # MoE模式下使用这个维度
                if SpikeDrivenTransformer._print_count == 0:
                    print("7. After mean:", x.shape)
                    SpikeDrivenTransformer._print_count += 1
