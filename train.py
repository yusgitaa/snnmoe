    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if hasattr(model, 'get_aux_loss'):
                ...
                moe_loss = model.module.get_aux_loss()
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                _logger.info(
                    ...
                    'MoE Loss: {moe_loss.val:#.4g} ({moe_loss.avg:#.3g})  '
                ...
                if writer is not None:
                    ...
                    writer.add_scalar('Train/MoE_Loss', moe_aux_losses_m.val, num_updates)
                    writer.add_scalar('Train/LR', lr, num_updates)
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if len(input.shape) == 4:
                input = input.unsqueeze(1).repeat(1, args.time_steps, 1, 1, 1)
            
            with amp_autocast():
                # 在这里预先初始化reduced_loss
                reduced_loss = torch.tensor(0.0, device=input.device)
                
                output = model(input)
                
                # 处理模型输出
                if isinstance(output, (tuple, list)):
                    # MoE模型可能返回(output, aux_loss)
                    output = output[0]
                ...
                if isinstance(loss, (tuple, list)):
                    # 如果loss是tuple/list，取第一个元素作为主要损失
                    reduced_loss = loss[0]
                else:
                    reduced_loss = loss
