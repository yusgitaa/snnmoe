...
def train_one_epoch(
    ...
    last_idx = len(loader) - 1  # 获取最后一个batch的索引

    second_order = hasattr(optimizer, "is_second_order") and optimizer.is_second_order
    batch_time_m = AverageMeter()
    data_time_m = AverageMeter()
    losses_m = AverageMeter()
    moe_aux_losses_m = AverageMeter()  # 添加MoE辅助损失的记录器

    model.train()
    functional.reset_net(model)

    print_once = True  # 添加一个标志，只打印一次调试信息
    
    end = time.time()
    for batch_idx, (input, target) in enumerate(loader):
        last_batch = batch_idx == last_idx  # 添加这行，定义 last_batch
        ...
        if print_once:  # 只在第一个批次打印调试信息
            print("\nTrain Debug Info:")
            print("1. Original input shape:", input.shape)
            print("1a. Target shape:", target.shape)
        ...
        if not args.prefetcher or args.dataset in dvs_utils.DVS_DATASET:
            ...
            if len(input.shape) == 4:  # [B, C, H, W]
                B = input.shape[0]
                if print_once:
                    print("2. Adding time dimension")
                input = input.unsqueeze(1)  # [B, 1, C, H, W]
                input = input.repeat(1, args.time_steps, 1, 1, 1)  # [B, T, C, H, W]
            
            if print_once:
                print("3. Before augmentation:", input.shape)
            
            if dvs_aug is not None:
                input = dvs_aug(input)
                if print_once:
                    print("4. After dvs_aug:", input.shape)
            
            if dvs_trival_aug is not None:
                if print_once:
                    print("5. Before dvs_trival_aug:", input.shape)
                input = dvs_trival_aug(input)
                if print_once:
                    print("6. After dvs_trival_aug:", input.shape)
            
            if print_once:
                print("7. Final input shape:", input.shape)
                print("7a. Final target shape:", target.shape)
            
            if mixup_fn is not None:
                input, target = mixup_fn(input, target)
                if print_once:
                    print("8. After mixup - input:", input.shape)
                    print("8a. After mixup - target:", target.shape)

        with amp_autocast():
            output, aux_info = model(input)
            if print_once:
                print("9. Model output shape:", output.shape)
                print("9a. Target shape for loss:", target.shape)
                print_once = False  # 关闭调试信息打印
            ...
            if isinstance(aux_info, dict) and "moe_aux_loss" in aux_info:
                moe_aux_loss = aux_info["moe_aux_loss"]
                if moe_aux_loss is not None:
                    moe_loss = moe_aux_loss
                    loss = loss + moe_loss
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.local_rank == 0:
                _logger.info(
                    f"Train: {epoch} [{batch_idx:>4d}/{last_idx}]  "
                    f"Loss: {losses_m.val:.4f} ({losses_m.avg:.4f})  "
                    f"MoE Loss: {moe_aux_losses_m.val:.6f} ({moe_aux_losses_m.avg:.6f})  "
                    f"Time: {batch_time_m.val:.3f}s, {sample_number * 1000 / batch_time_m.sum:>7.2f}/s  "
                    f"LR: {lr:.3e}  "
...
def validate(
    ...
    amp_autocast=suppress, log_suffix='', 
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            pred = pred.squeeze().cpu().numpy()  # 确保是1D数组
            all_preds.extend(pred)
            all_targets.extend(target.cpu().numpy())

    # 记录 TensorBoard 指标 (每个 epoch)
    if writer is not None and args.local_rank == 0 and epoch is not None:
        ...
        try:
            ...
            # 计算混淆矩阵
            cm = confusion_matrix(all_targets, all_preds, labels=range(args.num_classes))
            ...
            _logger.warning(f"无法生成混淆矩阵: {str(e)}")
