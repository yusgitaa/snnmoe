...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        # 保存原始图像用于可视化
        orig_images = input.clone().detach()
        if len(orig_images.shape) == 5:  # If in [T, B, C, H, W] format
            orig_images = orig_images[0]  # Take the first time step
        
        # 创建图像索引
        img_indices = torch.arange(input.size(0), device=input.device)
        ...
        with amp_autocast():
            ...
            if isinstance(output, tuple):
                ...
                if 'moe_aux_loss' in aux_info:
                    aux_loss = aux_info['moe_aux_loss']
                    loss = loss_fn(output, target) + args.aux_loss_alpha * aux_loss
                else:
            ...
            for i, module in enumerate(model.modules()):
                if hasattr(module, 'aux_loss') and hasattr(module, 'experts') and hasattr(module, '_expert_counts'):
                    ...
                    try:
                        ...
                        _logger.warning(f"无法记录专家使用情况: {e}")
