...
class DeepseekForSequenceClassification(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                sequence_lengths = torch.eq(input_ids, self.config.pad_token_id).int().argmax(-1)
            else:
                sequence_lengths = -1
...
class SpikeDeepseekMoE(nn.Module):
    def __init__(
        self,
        dim,
        hidden_dim,
        ...
        super().__init__()
        from .ms_conv import MS_MLP_Conv
        from spikingjelly.clock_driven import surrogate
        ...
        # 路由器网络
        self.gate = nn.Conv2d(dim, n_routed_experts, kernel_size=1, bias=False)
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Conv2d(dim, hidden_dim, 1),  # 256 -> 1024
                ...
                nn.Conv2d(hidden_dim, dim, 1),  # 1024 -> 256
                nn.BatchNorm2d(dim)
        ...
        # 打印专家结构
        print("\n" + "="*50)
        print("SpikeDeepseekMoE Structure:")
        print("-"*30)
        print("Router Network:")
        print(f"  Gate: Conv2d(in={dim}, out={n_routed_experts}, kernel=1)")
        
        print("\nExpert Network Structure (Each Expert):")
        print("  Sequential(")
        print("    1. First LIF:")
        print("       MultiStepLIFNode(tau=2.0, v_threshold=0.5, v_reset=None)")
        print("    2. First Conv:")
        print(f"       Conv2d(in={dim}, out={hidden_dim}, kernel=1)")
        print("    3. First BN:")
        print(f"       BatchNorm2d({hidden_dim})")
        print("    4. Second LIF:")
        print("       MultiStepLIFNode(tau=2.0, v_threshold=0.5, v_reset=None)")
        print("    5. Second Conv:")
        print(f"       Conv2d(in={hidden_dim}, out={dim}, kernel=1)")
        print("    6. Second BN:")
        print(f"       BatchNorm2d({dim})")
        print("  )")
        
        print("\nMoE Configuration:")
        print(f"  Number of Experts: {n_routed_experts}")
    ...
    def forward(self, x):
        ...
        for t in range(T):
            x_t = x[t]  # [B, C, H, W]
            logits_t = self.gate(x_t)  # [B, n_experts, H, W]
            routing_logits.append(logits_t)
        
        routing_logits = torch.stack(routing_logits)  # [T, B, n_experts, H, W]
        
        batch_size = T * B
        tokens_per_batch = H * W
        routing_logits_flat = routing_logits.reshape(batch_size, self.n_routed_experts, tokens_per_batch)
        
        router_z_loss = torch.logsumexp(routing_logits_flat, dim=1).pow(2).mean()
        routing_probs = F.softmax(routing_logits_flat, dim=1)
        
        if self.num_experts_per_tok < self.n_routed_experts:
            probs_transposed = routing_probs.transpose(1, 2)
            top_k_probs, top_k_indices = torch.topk(
                probs_transposed, k=self.num_experts_per_tok, dim=2
            )
            
            zeros = torch.zeros_like(probs_transposed)
            selected_probs = zeros.scatter_(2, top_k_indices, top_k_probs)
            
            routing_weights_flat = selected_probs.transpose(1, 2)
            routing_weights_flat = routing_weights_flat / (routing_weights_flat.sum(dim=1, keepdim=True) + 1e-9)
            
            total_expert_counts = torch.zeros(self.n_routed_experts, device=device)
            for expert_idx in range(self.n_routed_experts):
                expert_count = (top_k_indices == expert_idx).sum().float()
                total_expert_counts[expert_idx] = expert_count
            
            total_tokens = batch_size * tokens_per_batch * self.num_experts_per_tok
            expert_usage = total_expert_counts / total_tokens
            
            expert_usage_mean = expert_usage.mean()
            expert_usage_var = ((expert_usage - expert_usage_mean) ** 2).mean()
            load_balance_loss = expert_usage_var / (expert_usage_mean ** 2 + 1e-10)
            
            self.aux_loss = (router_z_loss * self.router_z_loss_coef + 
                           load_balance_loss * self.load_balance_coef)
            
            routing_weights = routing_weights_flat.reshape(T, B, self.n_routed_experts, H, W)
        else:
            routing_weights = F.softmax(routing_logits, dim=2)
            self.aux_loss = router_z_loss * self.router_z_loss_coef
        
        # 处理专家输出 - 修改这部分以正确处理5D输入
        ...
        for i in range(self.n_routed_experts):
            ...
            for t in range(T):
                x_t = x[t]  # [B, C, H, W]
                expert_output_t = self.experts[i](x_t)  # 处理单个时间步
                if isinstance(expert_output_t, tuple):
                    expert_output_t = expert_output_t[0]
                expert_outputs.append(expert_output_t)
            expert_output = torch.stack(expert_outputs)  # [T, B, C, H, W]
            
            # 应用路由权重
            expert_weight = routing_weights[:, :, i:i+1, :, :]  # [T, B, 1, H, W]
            expert_weight = expert_weight.expand(-1, -1, C, -1, -1)  # [T, B, C, H, W]
            outputs = outputs + expert_weight * expert_output
