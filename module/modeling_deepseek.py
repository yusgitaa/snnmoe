...
class SpikingExpert(nn.Module):
    def __init__(self, hidden_size, ffn_dim, spike_mode="lif", use_residual=False):
        super().__init__()
        # 输入维度是hidden_size，输出也是hidden_size
        ...
        # 打印专家信息用于调试
        print(f"SpikingExpert - hidden_size: {hidden_size}, ffn_dim: {ffn_dim}")
...
class DeepseekMoESparseMLP(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.hidden_size = config.hidden_size
        self.ffn_dim = config.moe_intermediate_size if hasattr(config, 'moe_intermediate_size') else config.intermediate_size
        ...
        # 添加可训练的软提示参数
        self.soft_prompt_dim = self.hidden_size
        self.use_soft_prompt = getattr(config, 'use_soft_prompt', False)
        if self.use_soft_prompt:
            # 初始化软提示
            self.soft_prompt = nn.Parameter(torch.randn(1, self.soft_prompt_dim) * 0.02)
            
            # 软提示投影到路由空间
            self.prompt_router_proj = nn.Linear(self.soft_prompt_dim, self.n_routed_experts, bias=False)
            
            # 平衡原始路由和提示路由的权重
            self.prompt_weight = nn.Parameter(torch.ones(1) * 0.1)  # 初始权重较小
            
            print(f"已添加软提示 - 维度: {self.soft_prompt_dim}")
        ...
        with torch.no_grad():
            ...
            if self.use_soft_prompt:
                nn.init.zeros_(self.prompt_router_proj.weight)
        
        self.register_buffer('_expert_counts', torch.zeros(self.n_routed_experts))
        self.aux_loss = None
        self.aux_loss_alpha = config.aux_loss_alpha if hasattr(config, 'aux_loss_alpha') else 0.1

        # 添加调试计数器
        self.register_buffer('_shared_expert_usage', torch.zeros(1))
        self.register_buffer('_step_counter', torch.zeros(1))

        if DEBUG_MOE:
            print(f"\nDeepseekMoESparseMLP初始化:")
            print(f"hidden_size: {self.hidden_size}")
            print(f"ffn_dim: {self.ffn_dim}")
            print(f"可路由专家数量: {self.n_routed_experts}")
            print(f"共享专家数量: {self.n_shared_experts}")
            print(f"每个token选择专家数: {self.top_k}")
            if self.use_soft_prompt:
                print(f"使用软提示: 是")

    def forward(self, hidden_states):
        """SNN的MoE前向传播 [T, B, C, H, W]"""
        # 输入维度检查
