...
def main():
    ...
    if args.act_scales is None:
        ...
        if 'mixtral' in args.net.lower():
            args.act_scales = f'./act_scales/Mixtral-8x7B-Instruct-v0.1_scales.pt'
        
    if args.act_shifts is None:
        args.act_shifts = f'./act_shifts/{args.net}.pt'
        if 'mixtral' in args.net.lower():
            args.act_shifts = f'./act_shifts/Mixtral-8x7B-Instruct-v0.1_shifts.pt'
    ...
    if args.wbits < 16 or args.abits <16:
        ...
        # 大约在第309行，为Mixtral添加特殊处理
        if 'mixtral' in args.net.lower() and args.nsamples > 32:
            # Mixtral模型更大，可能需要减少样本数
            logger.info(f"Mixtral模型较大，建议样本数从{args.nsamples}减少到32")
            # 可以选择自动减少样本数
            # args.nsamples = min(args.nsamples, 32)
